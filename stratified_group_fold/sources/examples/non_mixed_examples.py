...
def test_non_mixed_kfold():
    data = create_data(100, (1, 8), list(range(3)), 3, False)
    y = data["labels"]
    groups = data["groups"]
    X = data.drop(columns=["labels", "groups"])

    stratified_group_fold = StratifiedGroupKFold(n_splits=5, mixed_groups=False, opt_type=0)
...
def test_non_mixed_split():
    data = create_data(100, (1, 8), list(range(3)), 3, False)
    y = data["labels"]
    groups = data["groups"]

    splitter = StratifiedGroupSplit()
...
def test_flow():
    ...
    stratified_group_fold = StratifiedGroupKFold(n_splits=5, mixed_groups=False, opt_type=0, reset_index=True)
...
# test_non_mixed_split()
# test_non_mixed_kfold()
# test_flow()
